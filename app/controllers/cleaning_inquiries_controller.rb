...
class CleaningInquiriesController < ApplicationController
  ...
  def index
    @cleaning_inquiries = CleaningInquiry.order('id DESC').all.includes([:received_inquiry_response]) if user_signed_in?
  end
  ...
  def create
    ...
    respond_to do |format|
      @cleaning_inquiry.save ? flash_and_redirect(format) : format.html { render :new }
    end
  end
